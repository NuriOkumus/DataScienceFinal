...
<WebRConsole initialCode={`# 1. Modeli Kurma
library(MASS)
data(Pima.tr)

# Basit bir model: BMI tahmin et
# (Type hariç diğer hepsi)
...
<WebRConsole initialCode={`# 2. Diagnostics Plots (Sağlık Kontrolü)
# Ekranı 2x2 bölelim
par(mfrow = c(2, 2))
...
# İşimiz bitince ekranı düzeltelim
par(mfrow = c(1, 1))
`} />
...
<WebRConsole initialCode={`# 3. Stepwise Regression (Geriye Doğru Eleme)
# Gereksiz değişkenleri otomatik at
model_reduced <- step(model_full, direction = "backward")

# Bakalım kimler elendi, kimler kaldı?
